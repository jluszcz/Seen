...
CI (`.github/workflows/test.yml`, workflow "Build & Test") runs `format:check`, `lint`, `build`, and `test` on every push and PR to `main`. A Prettier pre-commit hook is also configured in `.pre-commit-config.yaml`.
